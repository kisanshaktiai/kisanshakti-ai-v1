...
/**
 * Farmer-facing NDVI record.
 *
 * Agronomic interpretation is deliberately absent here. The app consumes the
 * decision-grade DB view and only renders measurements, evidence and
 * provenance. TARKA/decision services own interpretation and actions.
 */
...
/** Kept as a compatibility type. Predictions are no longer generated client-side. */
...
export interface NDVIAnalysisResult {
  ...
  /** Always null until an authoritative backend prediction is available. */
...
const DECISION_VIEW = 'v_ndvi_decision_grade';
...
export function useNDVIAnalysis(landId: string | null): NDVIAnalysisResult {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Decision metrics ALWAYS come from the governed view. Raw ndvi_data is
      // queried only for non-semantic asset/provenance fields (image path,
      // metadata and legacy display metrics) after scene IDs are known.
      const [decisionResult, logResult] = await Promise.all([
        client.from(DECISION_VIEW)
          .select('land_id, tenant_id, acquisition_date, acquisition_time, scene_id, ndvi_value, savi_value, ndre_value, ndmi_value, uniformity_cv, quality_score, confidence_level, cloud_cover, observation_source, effective_pixel_count, coverage_weighted_purity, boundary_contamination_fraction, ndvi_spatial_se, evidence_confidence, measurement_status, spatial_stat_method, age_days, is_fresh')
          .eq('land_id', landId)
          .eq('tenant_id', tenantId)
          .gte('acquisition_date', cutoffDay)
          .order('acquisition_date', { ascending: false })
          .limit(60),
        client.from('ndvi_processing_logs')
      ...
      const decisionRows = (decisionResult.data || []) as any[];
      const sceneIds = decisionRows.map((r) => r.scene_id).filter(Boolean);
      let assetRows: any[] = [];
      if (sceneIds.length) {
        const assetResult = await client.from('ndvi_data')
          .select('land_id, tenant_id, date, scene_id, image_url, metadata, evi_value, ndwi_value, min_ndvi, max_ndvi, mean_ndvi, median_ndvi, ndvi_std, valid_pixels, total_pixels, satellite_source, collection_id, processing_level, spatial_resolution, tile_id, created_at, updated_at, computed_at, soil_moisture')
          .eq('land_id', landId)
          .eq('tenant_id', tenantId)
      ...
      const assetsByScene = new Map<string, any>();
      for (const row of assetRows) {
        if (row.scene_id && !assetsByScene.has(row.scene_id)) assetsByScene.set(row.scene_id, row);
      }

      const parsed = decisionRows.map((d: any, index: number) => {
        const a = assetsByScene.get(d.scene_id) || {};
        const metadata = a.metadata ? (typeof a.metadata === 'string' ? JSON.parse(a.metadata) : a.metadata) : null;
        return {
          ...a,
          ...d,
          id: `${d.land_id}:${d.scene_id || d.acquisition_date}:${index}`,
          date: d.acquisition_date,
          cloud_coverage: d.cloud_cover ?? null,
      ...
      const history = parsed.filter((r) => r.is_fresh === true);
      ...
      const processingThumbnail = successfulThumb?.metadata?.thumbnail_url
        ? { url: successfulThumb.metadata.thumbnail_url, date: successfulThumb.completed_at || successfulThumb.created_at || cutoffDay, geotiffUrl: successfulThumb.metadata.geotiff_url }
        : null;
...
/** Legacy compatibility hook. New farmer surfaces should consume decision-grade data. */

...
export function computeLandAnalytics(
  ...
  const priceRow =
    opts.market.find(
      (m) =>
        (m.commodity_name_normalized || m.crop_name || '').toLowerCase().includes(cropLc) && (m.modal_price || m.price_per_unit),
    ) || opts.market[0];
  ...
  const rainfallOffsetL = (opts.weather?.rain_24h_mm || 0) * 4046.86 * area; // 1mm/ha = 10,000L → /acre
  const waterRequirementL = Math.max(0, daily * area * 7 - rainfallOffsetL);

  // Recommendations
  const recs: string[] = [];
  if (latestNdvi != null && latestNdvi < 0.35) {
    recs.push('recommendations.low_ndvi');
  }
  if ((opts.weather?.rain_24h_mm ?? 0) < 1 && waterRequirementL > 0) {
    recs.push('recommendations.irrigate_soon');
  }
  if (nutrientLevel(opts.soil?.nitrogen_kg_per_ha ?? null, 'N') === 'low') {
    recs.push('recommendations.low_nitrogen');
  }
  if (delayed > 0) {
    recs.push('recommendations.tasks_delayed');
  }
  if (opts.soil?.ph_level != null && (opts.soil.ph_level < 5.5 || opts.soil.ph_level > 8.2)) {
    recs.push('recommendations.ph_out_of_range');
  }
  ...
  return {
    ...
    finance: { totalExpense, byCategory, income },
    marketPrice,
...
export function aggregateFarm(items: LandAnalytics[]): FarmAggregate {
  ...
  const totalExpense = items.reduce((s, i) => s + i.finance.totalExpense, 0);

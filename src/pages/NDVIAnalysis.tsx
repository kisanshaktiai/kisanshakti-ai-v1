...
export default function NDVIAnalysis() {
  ...
  const { current, history, latestRaw, isLoading, refetch } = useNDVIAnalysis(selectedLandId);
  ...
  return <div className="min-h-full bg-background flex flex-col">
    ...
    <Tabs value={tab} onValueChange={v => setTab(v as any)} className="flex-1 flex flex-col"><div className="px-3 pt-2"><TabsList className="w-full grid grid-cols-3 h-10 rounded-xl bg-muted/40"><TabsTrigger value="now" className="rounded-lg text-xs">{t('ndvi.tab.now','Now')}</TabsTrigger><TabsTrigger value="trend" className="rounded-lg text-xs"><BarChart3 className="h-3.5 w-3.5" /> {t('ndvi.tab.trend','Trend')}</TabsTrigger><TabsTrigger value="map" className="rounded-lg text-xs"><MapIcon className="h-3.5 w-3.5" /> {t('ndvi.tab.map','Map')}</TabsTrigger></TabsList></div>
      <TabsContent value="now" className="flex-1 px-3 pt-3 pb-24 space-y-3 mt-0">{isLoading?<Skeleton className="h-40 rounded-2xl" />:!current?<NoData/>:<>
        <Card className="rounded-2xl border-border/40"><CardContent className="p-4"><div className="flex items-center gap-4"><ObservationRing ndvi={current.ndvi_value}/><div className="flex-1 min-w-0"><p className="text-[10px] uppercase tracking-wider text-muted-foreground">{t('ndvi.now.observed','Observed NDVI')}</p><p className="text-lg font-bold">{current.ndvi_value.toFixed(2)}</p><p className="text-[10px] text-muted-foreground flex items-center gap-1 mt-1"><Calendar className="h-3 w-3" />{new Date(current.date).toLocaleDateString()}</p><p className="text-[9px] text-muted-foreground mt-1">{t('ndvi.provenance.source','Source')}: {current.observation_source || current.satellite_source || '—'}</p></div></div></CardContent></Card>
        ...
        <Card className="rounded-2xl border-border/40"><CardContent className="p-3"><p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-2">{t('ndvi.now.field_stats','Field statistics')}</p><div className="grid grid-cols-3 gap-2"><Mini label="Min" value={current.ndvi_value != null ? (current as any).ndvi_min : null}/><Mini label="Mean" value={current.ndvi_value}/><Mini label="Max" value={current.ndvi_value != null ? (current as any).ndvi_max : null}/></div></CardContent></Card>
      </>}</TabsContent>
      ...
      <TabsContent value="map" className="flex-1 px-0 pt-1 pb-16 mt-0">{selectedLandId&&<NDVIMapView landId={selectedLandId} boundary={boundary} centerLat={centerPoint.lat} centerLng={centerPoint.lng} areaAcres={selectedLand?.area_acres} currentCrop={selectedLand?.current_crop} landThumbnailUrl={selectedLand?.ndvi_thumbnail_url} landThumbnailDate={selectedLand?.last_ndvi_calculation}/>}<div className="px-3 pt-3"><SatelliteWaterLayerPanel layers={waterLayers} selectedCode={waterLayer} onSelect={setWaterLayer} loading={waterLayersLoading} error={waterLayersError}/></div></TabsContent>
    </Tabs></div>;
}

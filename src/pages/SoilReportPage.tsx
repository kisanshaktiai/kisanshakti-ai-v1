...
export default function SoilReportPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch soil health data with farmer and land details
      ...
      if (!data) {
        toast({ 
          title: 'No Data', 
          description: 'No soil health data available for this land',
          variant: 'destructive' 
        });
        navigate(-1);
        return;
      ...
      setSoilData(data);
      setFarmerData(data.user_profiles as any);
      setLandData(data.lands as any);
    } catch (error) {
      console.error('Error fetching soil data:', error);
      toast({ 
        title: 'Error', 
        description: 'Failed to load soil health data',
        variant: 'destructive' 
      });
    } finally {
      setLoading(false);
  ...
  const calculateHealthScore = (): number => {
    ...
    // Organic carbon score (30 points)
  ...
  const getNPKColor = (level: string): string => {
  ...
  const shareReport = async () => {
    ...
    const shareText = `🌱 Soil Health Report\n\nFarmer: ${farmerData.full_name}\nLand: ${landData.name}\nHealth Score: ${score}% (${status.text})\n\npH: ${soilData.ph_level}\nOrganic Carbon: ${soilData.organic_carbon}%\nNPK Levels: ${soilData.nitrogen_level}/${soilData.phosphorus_level}/${soilData.potassium_level}`;
    
    try {
      if (navigator.share) {
        await navigator.share({
          title: `Soil Health Report - ${landData.name}`,
          text: shareText
        });
      } else {
        await navigator.clipboard.writeText(shareText);
        toast({ title: 'Copied', description: 'Report details copied to clipboard' });
      }
    } catch (error) {
        ...
        <Card className="max-w-md glassmorphism">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Soil health data not found for this land
            </p>
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        ...
        <div ref={reportRef} className="space-y-4">
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4">
                  <Avatar className="h-16 w-16 border-2 border-primary/20">
                    <AvatarImage src={farmerData.avatar_url} alt={farmerData.full_name} />
                ...
                <div className="mt-4 pt-4 border-t border-border/50">
                  <p className="text-sm text-muted-foreground">
                    {[farmerData.address_line1, farmerData.village, farmerData.district, farmerData.state, farmerData.pincode]
                      .filter(Boolean)
                      .join(', ')}
                  </p>
                </div>
          ...
          <motion.div
            ...
            ref={cardRef}
          >
            <Card className={`glass-card-premium overflow-hidden border-2 bg-gradient-to-br ${getScoreBgColor(score)}`}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="relative">
                    ...
                    <Activity className={`h-20 w-20 relative ${getScoreColor(score)}`} />
                  </div>
                </div>

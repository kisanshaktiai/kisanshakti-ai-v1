...
interface Props {
  landId: string;
  layer: SatelliteWaterLayerView | null;
  height?: string;
}

/**
 * Geospatial evidence viewer. Unlike a plain <img>, the raster is anchored to
 * the pipeline's WGS84 bounds and therefore stays attached to the real field
 * while the farmer pans/zooms the map.
 */
export function SatelliteWaterLayerMap({ landId, layer, height = '260px' }: Props) {
  ...
  useEffect(() => {
    ...
    // boundary is synchronized below; initialize once.
  ...
  useEffect(() => {
    ...
    const b = metadataBounds(layer) ?? polygonBounds(boundary)?.reduce((acc, v) => acc, null as any);
    if (!b || !('west' in b)) return;
    map.addSource('water-evidence-image', {
      type: 'image',
      url: layer.signedImageUrl,
      coordinates: [[b.west, b.north], [b.east, b.north], [b.east, b.south], [b.west, b.south]],
    });
    map.addLayer({
      id: 'water-evidence-raster',
      type: 'raster',
      source: 'water-evidence-image',
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-border/40 bg-muted/20" style={{ height }}>
      ...
      {!layer?.signedImageUrl && !boundaryError && (
      ...
      {boundaryError && (
        <div className="absolute left-2 right-2 bottom-2 rounded-lg bg-background/90 px-2 py-1 text-[10px] text-destructive">
          {boundaryError}
        </div>
      )}
      <div className="absolute right-2 top-2 z-10 rounded-lg bg-background/90 px-2 py-1 text-[10px] font-medium shadow-sm">
        Field location · satellite evidence
      </div>
    </div>
  );
}

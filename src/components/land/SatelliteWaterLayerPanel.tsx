...
export function SatelliteWaterLayerPanel({ layers, selectedCode, onSelect }: Props) {
  const latest = useMemo(() => layers.find((item) => item.layer_code === selectedCode) ?? null, [layers, selectedCode]);
        ...
        <>
          {latest.signedImageUrl && (
            <div className="overflow-hidden rounded-lg border bg-muted/20">
              <img src={latest.signedImageUrl} alt={LABELS[selectedCode]} className="block h-auto w-full object-contain" loading="lazy" />
            </div>
          )}
          <div className="text-xs text-muted-foreground">
            ...
            <div className="mt-1">Satellite evidence only; not a confirmed water-stress diagnosis.</div>
          </div>
        </>
      ) : (

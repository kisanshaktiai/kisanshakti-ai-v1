...
Deno.test("production Harness receives the exact resolved request context", async () => {
  ...
  assert(harness.includes("contextSnapshot"));
  assert(harness.includes("legacy.resolvedInputs"));
  assert(llm.includes("resolved_context"));
});

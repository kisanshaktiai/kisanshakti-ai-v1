...
// Names that would indicate hardcoded agronomy sneaking back in.
const BANNED_IDENTIFIERS = [
  ...
  "CROP_SPECIFIC_PRODUCTS",
  "EXPERT_NPK_SPLIT_SCHEDULE",
  "FALLBACK_TASK_TEMPLATES",
  "YIELD_BOOST_TECHNIQUES",
  "CROP_TASK_MULTIPLIERS",
  "LABOR_REQUIREMENTS",
];
...
Deno.test("no hardcoded agronomy tables in the schedule pipeline", async () => {
  for (const f of FILES) {
    ...
    for (const id of BANNED_IDENTIFIERS) {
      assert(!src.includes(`${id}:`) && !src.includes(`const ${id}`), `${f} reintroduced ${id}`);
    }
  }
});

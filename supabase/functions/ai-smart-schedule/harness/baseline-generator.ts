// CHANGE LOG
// 2026-09-07 — validator receives each stage's sowing-axis das_min so graph-declared pre-season stages
//   (negative das) are admitted from the SSOT rather than rejected (V3).
...
export async function generateBaseline(
  ...
  const validation = validateBaseline(
    ...
    stages.map((s) => ({ id: s.id, das_max: das0(s, s.das_max), das_min: das0(s, s.das_min) })),

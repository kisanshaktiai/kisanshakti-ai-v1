...
serve(async (req) => {
  ...
  try {
    ...
    if (baseline.validation && baseline.validation.warnings.length) {
      for (const w of baseline.validation.warnings) {
        baseline.gaps.push(`validation_warning: ${w}`);
      }
    }

    // ── HARNESS V2: LLM planner over deterministic candidate graph ──────────
    // The model may only order existing candidates. Validation failure falls back
    // deterministically inside applyScheduleHarness; no agronomic candidate is invented.
    ...
    try {
      ...
      return json({
        error: "Schedule harness execution failed before persistence",

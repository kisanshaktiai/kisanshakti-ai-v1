...
const NUM_RE = /\d+(?:[.,]\d+)?/g; const CHUNK_SIZE = 5; const MAX_CONCURRENCY = 2; const NARRATION_BUDGET_MS = 100_000; const RETRY_DELAYS_MS = [2_000, 6_000, 15_000]; const MAX_RETRY_AFTER_MS = 20_000; let rateLimited = false;
// Provider 429s are per-provider (TPM/RPM budgets are workspace-wide for ONE vendor), so the
// cooldown is keyed by provider: a rate-limited primary immediately yields to the fallback
// provider instead of parking every chunk behind one global wait window.
...
async function narrateChunk(chunk: NarratableTask[], offset: number, language: string, signal: AbortSignal): Promise<{ items: Array<{ i: number; name?: string; desc?: string; instructions?: string[] }>; provider: AIProvider; model: string }> {
  ...
  const chain = getScheduleProviderChain().filter((p) => getAPIKey(p.provider));
  // Prefer a provider that is not currently cooling down; only wait when every provider is limited.
...
export async function narrateTasks(tasks: NarratableTask[], language: string): Promise<{ tasks: NarratableTask[]; narrated: boolean; narratedCount: number; totalCount: number; appliedIndices: number[]; reason?: string; provider?: string; model?: string }> {
  ...
  try {
    ...
    while (i < chunks.length) { if (controller.signal.aborted) break; const width = rateLimited ? 1 : MAX_CONCURRENCY; results.push(...await Promise.allSettled(chunks.slice(i, i + width).map((c) => narrateChunkWithRetry(c.items, c.offset, language, controller.signal)))); i += width; }
    for (const result of results) {
      ...
      for (const item of result.value.items) { const target = out[item.i]; if (!target) continue; let touched = false;
  ...
  // Fail closed: index.ts persists only after this function returns. Throwing here prevents
  // an incomplete non-English narration from reaching persistence and makes the API error explicit.
  if (language !== "en" && applied < totalCount) throw new Error(`FARMER_LANGUAGE_UNAVAILABLE:${applied}/${totalCount}:${[...new Set(failures)].slice(0, 2).join("|") || "translation_validation_failed"}`);

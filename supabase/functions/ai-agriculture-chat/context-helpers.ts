...
export function getMinimalContext(
  ...
  if (queryType === 'watering') {
    return `${cropName}(${daysSinceSowing}d) | ${areaInAcres}ac | ${land.irrigation_type || 'No irrig'}`;
  }
  ...
  if (queryType === 'fertilizer') {
    ...
    return `${cropName} ${areaInAcres}ac | NPK: N:${n} P:${p}${p !== '?' && Number(p) < 20 ? '↓' : ''} K:${k}`;
  }
  
  if (queryType === 'pest') {
    const ndviVal = latestNDVI?.ndvi_value || latestNDVI?.mean_ndvi || '?';
    const healthStatus = getHealthStatus(ndviVal);
    return `${cropName}(${daysSinceSowing}d) | NDVI:${ndviVal}(${healthStatus})`;
  }
  
  if (queryType === 'health') {
    const ndviVal = latestNDVI?.ndvi_value || latestNDVI?.mean_ndvi || '?';
    return `${cropName}(${daysSinceSowing}d) | NDVI:${ndviVal}${ndviVal !== '?' && Number(ndviVal) >= 0.7 ? '↑' : ''}`;
  }
  
  if (queryType === 'market') {
    return `${cropName} ${areaInAcres}ac | Stage: ${daysSinceSowing}d`;
  }
  
  // General: minimal info
  return `${cropName} ${areaInAcres}ac`;
}

...
CREATE OR REPLACE FUNCTION private.current_farmer_owns_land(
  p_land_id uuid,
  p_tenant_id uuid
)
RETURNS boolean
...
REVOKE ALL ON FUNCTION private.current_farmer_owns_land(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.current_farmer_owns_land(uuid, uuid) TO anon, authenticated, service_role;
...
USING (
  ...
  OR private.current_farmer_owns_land(land_id, tenant_id)
...
USING (
  ...
  AND (
    private.current_farmer_owns_land(
...
REVOKE ALL ON FUNCTION public.current_farmer_owns_land(uuid, uuid) FROM PUBLIC, anon, authenticated, service_role;
DROP FUNCTION public.current_farmer_owns_land(uuid, uuid);
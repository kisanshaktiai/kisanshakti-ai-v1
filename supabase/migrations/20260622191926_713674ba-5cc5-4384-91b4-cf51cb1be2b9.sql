...
WITH base AS (
  SELECT
    ...
    -- Wave K: prefer the runtime-stamped disposition; fall back to the
    -- canonical site→disposition map mirrored from
    -- utils/clarification-site-tag.ts (single source of truth: keep in sync).
    COALESCE(
      ...
      CASE m.metadata->>'clarification_site'
        ...
        WHEN 'orch.dynamic_options'                  THEN 'DEFECT_SUSPECT'
  ...
  SELECT
    ...
    CASE
      ...
      -- Wave K: dispositions override generic buckets so by-design questions
      -- stop polluting the pre_brain_clarification defect signal.

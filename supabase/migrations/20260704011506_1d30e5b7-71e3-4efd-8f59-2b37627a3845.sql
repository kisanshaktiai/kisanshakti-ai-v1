...
DECLARE
  ...
  v_new_stage          record;
BEGIN
  -- Alias `lands` as l so column names cannot collide with RETURNS TABLE OUT names
  ...
  -- Alias csm — crop_code / crop_cycle in table would collide with OUT names
  SELECT csm.* INTO v_stage
    FROM public.crop_stage_master csm
   WHERE csm.is_active
     AND csm.crop_code = v_crop_code
     AND coalesce(csm.crop_cycle,'') IN (v_crop_cycle, '')
     AND csm.stage_node_type IS DISTINCT FROM 'operational'
     AND v_das BETWEEN coalesce(csm.das_min, 0) AND coalesce(csm.das_max, 9999)
   ORDER BY (csm.crop_cycle = v_crop_cycle) DESC NULLS LAST, csm.das_min ASC
   LIMIT 1;
  IF NOT FOUND THEN RETURN; END IF;

  -- ─── Phase E: transition override ─────────────────────────────────────
  -- `confidence` in the returned table collides with local var — alias explicitly.
  ...
  -- Variety override (alias vpp — crop_code/crop_cycle/stage_uuid collide with OUT)
  IF v_variety_id IS NOT NULL THEN
    SELECT vpp.* INTO v_vpp
      FROM public.variety_phenology_profile vpp
     WHERE vpp.is_active
       AND vpp.crop_code = v_crop_code
       AND vpp.variety_id = v_variety_id
       AND vpp.stage_uuid = v_stage.id
       AND coalesce(vpp.crop_cycle,'') IN (v_crop_cycle, '')
     ORDER BY (vpp.crop_cycle = v_crop_cycle) DESC NULLS LAST
     LIMIT 1;
    IF FOUND THEN

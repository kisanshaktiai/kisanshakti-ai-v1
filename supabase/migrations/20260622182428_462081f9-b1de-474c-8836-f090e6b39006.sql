...
DROP VIEW IF EXISTS public.v_ai_drop_attribution_90d;

CREATE VIEW public.v_ai_drop_attribution_90d AS
SELECT
  ...
  COALESCE(metadata->>'clarification_origin', 'unknown')     AS clarification_origin,
  COUNT(*)                                                   AS turns,
  COUNT(*) FILTER (
...
COMMENT ON VIEW public.v_ai_drop_attribution_90d IS
  'WS13 Wave E/F: per-crop, per-response-type, per-drop-stage, per-clarification-origin attribution of the match-to-decision pipeline. Service-role only.';

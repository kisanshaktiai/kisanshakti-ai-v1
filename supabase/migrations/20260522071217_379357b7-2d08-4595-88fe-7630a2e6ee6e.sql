...
-- ============================================================
-- A. Auto-downgrade job (in-place UPDATE — one row per farmer)
-- ============================================================
CREATE OR REPLACE FUNCTION public.expire_farmer_subscriptions()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _free_plan_id IS NULL THEN
    RAISE EXCEPTION 'Free plan not found; cannot auto-downgrade';
  END IF;

  FOR _r IN
    SELECT id, farmer_id, tenant_id, plan_id, status, end_date, grace_period_ends_at, metadata
    FROM public.farmer_subscriptions
    WHERE plan_id <> _free_plan_id
      AND status IN ('active','trial','trialing','expired')
      AND end_date IS NOT NULL
      AND end_date < now()
      AND (grace_period_ends_at IS NULL OR grace_period_ends_at < now())
  LOOP
    UPDATE public.farmer_subscriptions
       SET plan_id    = _free_plan_id,
           status     = 'active',
           start_date = now(),
           end_date   = NULL,
           ...
           metadata = COALESCE(metadata,'{}'::jsonb) || jsonb_build_object(
             ...
             'auto_downgraded_from_plan_id', _r.plan_id,
             'auto_downgraded_from_end_date', _r.end_date,
             'auto_downgrade_reason', 'subscription_expired'
           ),
           updated_at = now()
     WHERE id = _r.id;
    ...
    BEGIN
      INSERT INTO public.subscription_change_history
        (farmer_id, tenant_id, from_plan_id, to_plan_id, change_type, change_reason, created_at)
      VALUES
        (_r.farmer_id, _r.tenant_id, _r.plan_id, _free_plan_id,
         'auto_downgrade', 'subscription_expired', now());
    EXCEPTION WHEN OTHERS THEN
      NULL; -- never block downgrade on history-table schema differences
    END;
  ...
  RETURN jsonb_build_object(
    'downgraded_to_free', _downgraded_count,
    'ran_at', now()
  );
END;
$$;

REVOKE ALL ON FUNCTION public.expire_farmer_subscriptions() FROM PUBLIC, anon, authenticated;

-- ============================================================
-- B. Fixed entitlements resolver — Free fallback + no "default true"
-- ============================================================
...
BEGIN
  ...
  -- CRITICAL: final fallback is now `false`, not `true`.
  ...
  FROM (
    SELECT
      ...
      jsonb_build_object(
        'enabled', COALESCE(
          CASE WHEN _tenant_features ? f.code THEN
            CASE jsonb_typeof(_tenant_features->f.code)
              WHEN 'boolean' THEN (_tenant_features->>f.code)::boolean
              ELSE NULL END
          ELSE NULL END,
          tfpf.enabled,
          tfg.enabled,
...
-- ============================================================
-- C. Schedule hourly job + run once to clean current stale rows
-- ============================================================
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname='pg_cron') THEN
    IF EXISTS (SELECT 1 FROM cron.job WHERE jobname='expire-farmer-subscriptions-hourly') THEN
      PERFORM cron.unschedule('expire-farmer-subscriptions-hourly');
    END IF;
    PERFORM cron.schedule(
      'expire-farmer-subscriptions-hourly',
      '0 * * * *',
      $cron$ SELECT public.expire_farmer_subscriptions(); $cron$
    );
  END IF;
END $$;

SELECT public.expire_farmer_subscriptions();
